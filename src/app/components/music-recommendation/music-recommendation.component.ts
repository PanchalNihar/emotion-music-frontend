...
import { MusicRecommendationResponse,MusicRecommendationService } from '../../services/music-recommendation.service';
...
export class MusicRecommendationComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    if (file.size > 10 * 1024 * 1024) {
      this.errorMessage = 'Image file is too large. Please select a file smaller than 10MB.';
      return;
    }
  ...
  onTextSearch(mood: string): void {
    ...
    if (cleanMood.length < 2) {
      this.errorMessage = 'Please enter at least 2 characters to describe your mood.';
      return;
    }
    ...
    return {
      next: (response: MusicRecommendationResponse) => {
        ...
        if (!response.tracks || response.tracks.length === 0) {
          this.errorMessage = 'No tracks found for your mood. Please try a different search.';
          this.results = null;
        }
      },
      error: (error: HttpErrorResponse) => {
        ...
        if (error.status === 0) {
          this.errorMessage = 'Unable to connect to the service. Please check your internet connection and try again.';
